...
const JobOngoing = () => {
  ...
  const latestChange = priceChanges.length > 0 ? priceChanges[priceChanges.length - 1] : null;
  ...
  // ✅ Total cumulative approved delta from original
  ...
  // ✅ Dynamic Paid amount (includes 1st, 2nd, final payments)
  const paid =
    (bd.firstPayment?.status === "paid" ? bd.firstPayment.amount || 0 : 0) +
    (bd.secondPayment?.status === "paid" ? bd.secondPayment.amount || 0 : 0) +
    (bd.finalPayment?.status === "paid" ? bd.finalPayment.amount || 0 : 0);

  // ✅ Dynamic yet-to-pay
  const totalToCompare =
    isPending && pendingTotal
      ? pendingTotal
      : isApproved
        ? approvedTotal
        : bd.finalTotal || approvedTotal || originalTotal;

  const amountYetToPay = Math.max(totalToCompare - paid, 0);
  ...
  const disableAllTheHell = bd.paymentStatus === 'paid' && isProjectCompleted;
                    ...
                    {isApproved && (
                      <View>
                        <View style={styles.rowBetween}>
                          ...
                          <Text style={styles.amountDue}>{currency(originalTotal)}</Text>
                        </View>

                        <View style={styles.rowBetween}>
                          <Text style={styles.amountLabel}>Change (Approved)</Text>
                          <Text
                            style={[
                              styles.amountDue,
                        ...
                        <View style={styles.rowBetween}>
                          ...
                          <Text style={styles.amountDue}>{currency(approvedTotal)}</Text>
                        </View>
                        ...
                        {bd.secondPayment?.status === "paid" && (
                          <Text style={{ color: "#666", fontSize: 12, marginBottom: 4 }}>
                            (1st: {currency(bd.firstPayment.amount)} + 2nd:{" "}
                            {currency(bd.secondPayment.amount)})
                          </Text>
                        )}

                        <View style={styles.rowBetween}>
                          <Text style={styles.amountLabel}>Amount Yet to be Paid</Text>
                          <Text style={styles.amountDue}>{currency(amountYetToPay)}</Text>
                        </View>
                      </View>
                    )}

                    {/* 🟠 CASE: PENDING (Customer/Admin Approval Waiting) */}
                    {isPending && (
                      <View>
                        {hasApprovedChange ? (
                          <>
                            ...
                            <View style={styles.rowBetween}>
                              <Text style={styles.amountLabel}>New Total (Approved)</Text>
                              <Text style={styles.amountDue}>{currency(approvedTotal)}</Text>
                            </View>
                            ...
                            <View style={styles.rowBetween}>
                              <Text style={styles.amountLabel}>Change (Pending)</Text>
                              <Text
                                style={[
                                  styles.amountDue,
                            ...
                            <View style={styles.rowBetween}>
                              <Text style={styles.amountLabel}>New Total (Pending)</Text>
                              <Text style={styles.amountDue}>{currency(pendingTotal)}</Text>
                            </View>
                    ...
                    {isRejected && (
                      <View>
                        ...
                        <View style={styles.rowBetween}>
                          ...
                          <Text style={[styles.amountDue, { color: "red" }]}>Rejected</Text>
                        </View>
                      </View>
                    )}

                    {/* ⚪ DEFAULT CASE (No Price Edits Yet) */}
                    {!isPending && !isApproved && !isRejected && (
                      <View>
                        <View style={styles.rowBetween}>
                          <Text style={styles.amountLabel}>Total Amount</Text>
                          <Text style={styles.amountDue}>{currency(originalTotal)} {" "}
                            <TouchableOpacity
