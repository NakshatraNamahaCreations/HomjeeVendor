...
export default function QuoteSummary() {
  ...
  const totalsFromLines = useMemo(() => {
    const norm = v =>
      String(v || '')
        .trim()
        .toLowerCase();
    const to = n => Number(n ?? 0);

    // sum lines where the checker returns true
    const sumWhere = checker =>
      (activeQuote?.lines || [])
        .filter(l => {
          const sec = norm(l.sectionType || l.section || l.category || l.type);
          return checker(sec, l);
        })
        .reduce((s, l) => s + to(l.subtotal ?? l.total), 0);

    const interior = sumWhere(sec => sec === 'interior');
    const exterior = sumWhere(sec => sec === 'exterior');
    const others = sumWhere(sec => sec === 'others' || sec === 'other');

    // be liberal with labels for safety: "Additional Services", "Additional Service",
    // "Additional", "Add-on/Addon", "Extras/Extra"
    const addl = sumWhere(
      sec =>
        sec.startsWith('additional') ||
        sec.includes('addon') ||
        sec === 'extras' ||
        sec === 'extra',
    );

    const subtotal = +Number(interior + exterior + others + addl).toFixed(2);

    return {
      interior,
      exterior,
      others,
      additionalServices: addl,
      subtotal,
    };
  }, [activeQuote]);
  ...
  const quoteTotalsFromLines = useMemo(() => {
    ...
    const subtotal = round2(
      interior + exterior + others + totalsFromServer?.additionalServices,
    );
    return { interior, exterior, others, subtotal };
  }, [activeQuote]);

  console.log('quoteTotalsFromLines', quoteTotalsFromLines);
  ...
  const subtotal = useMemo(() => {
    const s =
      (Number.isFinite(interiorTotal) ? interiorTotal : 0) +
      (Number.isFinite(exteriorTotal) ? exteriorTotal : 0) +
      (Number.isFinite(othersTotal) ? othersTotal : 0) +
      (Number.isFinite(additionalServices) ? additionalServices : 0);
    return +s.toFixed(2);
