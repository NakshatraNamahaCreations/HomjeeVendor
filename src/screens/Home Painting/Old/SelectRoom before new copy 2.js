...
const SelectRoom = ({ navigation }) => {
  ...
  // const renderFromQuoteLine = (room, line) => {
  //   const kinds =
  //     room.sectionType !== 'Others' ? ['Ceiling', 'Wall'] : ['Measurement'];

  //   const renderKind = type => {
  //     const items = pick(line, type).sort(sortByDisplay);
  ...
  //     const paint = getPaintFromLine(line, kindKey);
  //     const title =
  //       type === 'Measurement' ? room.name || 'Measurement' : `${type}s`;
  ...
  //         {items.map((b, i) => {
  //           // display as-is (sqft from backend), but amount uses saved unitPrice
  //           const surfaceLabel = labelForBreakdown(room, b); // e.g. "Wall 1"
  //           const amount = Number(
  //             (Number(b.sqft || 0) * Number(b.unitPrice || 0)).toFixed(2),
  //           );

  //           return (
  //             <View key={`${type}-${i}`} style={{ marginBottom: 6 }}>
  //               <Text style={styles.paintName}>
  //                 {composePaintLabel(room, paint, b.mode)}
  //               </Text>
  //               <View style={styles.detailRow}>
  //                 <Text style={styles.detailLabel}>{surfaceLabel}</Text>
  //                 <Text style={styles.detailValue}>{fmtSqft(b.sqft)}</Text>
  //                 <Text style={styles.detailValue}>{fmtMoney(amount)}</Text>
  //               </View>

  //               {/* Additional services for THIS surface go right below */}
  //               {renderAdditionalForSurface(room, surfaceLabel)}
      ...
      <View style={styles.container}>
          ...
          {groupedRooms[activeTab]?.map((room, index) => {
...

            {
              loadingQuote
                ...
                : pkgLine
                ? renderFromQuoteLine(room, pkgLine) // preview package totals
                : null;
            }
            return (
              <View key={index} style={styles.summaryCard}>
                <TouchableOpacity
                  ...
                  //     roomName: room.name,
                  //     existingRoom: room,
                  //     quoteId: quoteId, // save into the destination quote
                  //     existingLine: line || null, // prefill from quote lines (src/dst)
                  //     paintsCache,
                  //     onSaved: newLine => {
                  //       // Keep local view in sync (optional)
                  //       setActiveQuote(q => {
                  //         const lines = [...(q?.lines || [])];
                  //         const i = lines.findIndex(
                  ...
                  //       });
                  //     },
                  //     onSavedAdditional: (fromRoomName, respData) => {
                  //       const key = norm(fromRoomName || room.name);
                  //       setAdditionalByRoom(prev => ({
                  //         ...prev,
                  //         [key]: {
                  //           additionalServices:
                  //             respData?.additionalServices || [],
                  //           additionalTotal: Number(
                  //             respData?.additionalTotal || 0,
                  //           ),
                  //         },
                  //       }));
                  //     },
                  ...
                  //       // keep SelectRoom in sync immediately after save too
        ...
        <View style={styles.footer}>
          <TouchableOpacity
            ...
            //       .map(r => buildLineFromPackage(r, predefPackage))
            //       .filter(l => l && l.subtotal > 0); // skip empty

            //     // save sequentially (or Promise.all if your API can handle it)
            //     for (const l of built) {
            //       const saved = await saveRoomLine(quoteId, l);
            //       // keep local quote view in sync
            //       setActiveQuote(q => {
            //         const lines = [...(q?.lines || [])];
            //         const i = lines.findIndex(
            //           x => norm(x.roomName) === norm(saved.roomName),
