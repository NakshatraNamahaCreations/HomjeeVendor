...
const RoomMeasurementScreen = () => {
  ...
  // const [sections, setSections] = useState(null); // Start as null
  ...
  useEffect(() => {
    console.log('Final sections rendered:', sections);
  }, [sections]);
  ...
  //     // Hydrate data based on activeTab and room data
  //     const rawTab =
  //       tab ||
  ...
  //     // Return hydrated data based on tab and room
  ...
  //     // Hydration for Others and other custom logic
  //     return [];
  //   },
  ...
  const hydrateFromRoom = (tab, room, measurementType) => {
    const resolvedTab = (tab || 'Interior').trim();
    const modeFromRoom = room?.mode || MODE.REPAINT;
    ...
    switch (resolvedTab) {
      case 'Interior':
        return [
          makeCeiling('Ceiling 1'),
          ...Array.from({ length: 4 }, (_, i) => makeWall(`Wall ${i + 1}`)),
        ];
      case 'Exterior':
        return [
          makeCeiling('Ceiling 1'),
  ...
  // const hydrateFromRoom = useCallback((tab, room, measurementType) => {
  //   if (!room) return [];
  ...
  useEffect(() => {
    // If we already have sections from previous hydration, skip
    ...
    if (!picked?.room) {
      ...
      // No room found → use fallback defaults
      const fallback = buildDefaultSections(
        activeTab,
        MODE.REPAINT,
        type || 'Grills',
      );
    ...
    // Dependencies: run only once on mount or when these change
    etstimateData,
    nameOfTheRoom,
    type,
    activeTab,
    route?.params?.type,
    // Don't include `hydrateFromRoom` here since it's not memoized
  ]);
  ...
  const addCeiling = () => {
    ...
    const ceilingCount = Array.isArray(sections)
      ? sections.filter(s => s.type === 'ceiling').length
      : 0;
    setSections(prev => [
  ...
  const addWall = () => {
    const wallCount = Array.isArray(sections)
      ? sections.filter(s => s.type === 'wall').length
      : 0;
  ...
  const addExteriorCeiling = () => {
    const ceilCount = Array.isArray(sections)
      ? sections.filter(s => s.type === 'ceiling').length
      : 0;
    setSections(prev => [
      ...(prev || []),
  ...
  const addOtherItem = () => {
    ...
    const count = Array.isArray(sections)
      ? sections.filter(s => s.type === key).length
      : 0;
    setSections(prev => [
      ...(prev || []),
      {
