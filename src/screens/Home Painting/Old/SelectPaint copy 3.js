...
export default function SelectPaint() {
  ...
  //                 {service.materialName ? `${service.materialName}` : ''}
  //               </Text>
  ...
  const renderAdditionalServices = (roomName, surfaceTitle) => {
    ...
    return (
      <View>
        {servicesForThisLine.map((service, index) => {
          // If withPaint, show the actual unit price, else zero
          const effectiveUnitPrice = service.withPaint ? service.unitPrice : 0;
          const effectiveTotal = service.withPaint ? service.total : 0;
          ...
          return (
            <View key={`${service.serviceType}-${index}`}>
              <View style={styles.detailRow}>
                ...
                <Text style={styles.detailValue}>
                  {`${service.areaSqft} sq ft × ₹${effectiveUnitPrice} = ₹${effectiveTotal}`}
                </Text>
              </View>
              <Text
                style={{
