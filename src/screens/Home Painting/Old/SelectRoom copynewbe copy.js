...
const SelectRoom = () => {
  ...
  //   const all = pick(line, type).sort(sortByDisplay);
  //   const items = all;
  ...
  const summarizeFromLine = (room, line, type) => {
    // take what server saved, but drop zeroed rows (surfaces removed by "Without Paint")
  ...
  const renderFromQuoteLine = (room, line) => {
    ...
    const paintSubtotal = Number(
      line.subtotal ?? sum(line.breakdown), // fallback if subtotal missing
    );
    ...
    const renderAdditionalBlock = (room, line) => {
      ...
      // group by surface for a tidy layout
      const bySurface = {};
      for (const it of list) {
        const key = (it.surfaceType || 'General').trim();
        (bySurface[key] ||= []).push(it);
      }
      ...
      return (
        <View style={{ marginTop: 8 }}>
          ...
          {Object.entries(bySurface).map(([surface, items]) => (
            <View key={surface} style={{ marginTop: 4 }}>
              <Text style={[styles.detailLabel, { marginBottom: 2 }]}>
                {surface}
              </Text>

              {items.map((it, i) => (
                <View key={`${surface}-${i}`} style={styles.detailRow}>
                  <Text style={styles.detailLabel}>
                    {it.materialName || it.serviceType}
                  </Text>
                  ...
                  {/* keep it for later use if need */}
                  {/* <Text
                    style={{
                      marginLeft: 6,
                      fontSize: 11,
                      fontFamily: 'Poppins-SemiBold',
                      color: it.withPaint ? '#066a36' : '#b22222',
                    }}
                  >
                    ({it.withPaint ? 'With Paint' : 'Without Paint'})
                  </Text> */}
                  {/* <TouchableOpacity
                    style={{ marginLeft: 12 }}
                    onPress={() =>
                      navigation.navigate('AdditionalService', {
                        ...
                        editItem: it, // optional if you want deep-preset
                      })
                    }
                  >
                    <Text
                      style={{ color: '#007aff', fontFamily: 'Poppins-Medium' }}
                    >
                      Edit
                    </Text>
                  </TouchableOpacity>
                          ...
                          onPress: async () => {
                            try {
                              ...
                              setActiveQuote(q => {
                                ...
                                if (idx < 0) return q;
                                const L = { ...(lines[idx] || {}) };
                                const list = Array.isArray(L.additionalServices)
                                ...
                                const same = a =>
                                  ...
                                  (a.materialName || a.customName || '') ===
                                    (it.materialName || it.customName || '') &&
                                  (a.serviceType || '') ===
                                    (it.serviceType || '') &&
                                  (a.surfaceType || '') ===
                                    (it.surfaceType || '') &&
                                  !!a.withPaint === !!it.withPaint &&
                                  Number(a.areaSqft || 0) ===
                                    Number(it.areaSqft || 0) &&
                                  Number(a.unitPrice || 0) ===
                                    Number(it.unitPrice || 0);
                                const pos = list.findIndex(same);
                                if (pos >= 0) list.splice(pos, 1);
                                L.additionalServices = list;
                                L.additionalTotal = list.reduce(
                  ...
                  >
                    <Text
                      style={{ color: '#b22222', fontFamily: 'Poppins-Medium' }}
                    >
                      Delete
                    </Text>
                  </TouchableOpacity> */}
