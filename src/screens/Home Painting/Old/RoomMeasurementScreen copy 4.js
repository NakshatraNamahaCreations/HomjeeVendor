...
const RoomMeasurementScreen = () => {
  ...
  // Hydrate sections when data arrives
  useEffect(() => {
  ...
  const addExteriorCeiling = () => {
    const count = sections.filter(s => s.type === 'ceiling').length;
    setSections(prev => [
      ...prev,
      {
        type: 'ceiling',
        ...
        mode: roomDefaultMode,
        customMode: false,
        label: `Ceiling ${count + 1}`,
      },
    ]);
  };

  const addExteriorWall = () => {
    const count = sections.filter(s => s.type === 'wall').length;
    setSections(prev => [
      ...prev,
      {
        type: 'wall',
  ...
  const validateBeforeSave = () => {
    ...
    if (bad) {
      Alert.alert(
        'Missing dimensions',
        'Please enter valid length and width for all sections.',
      );
      return false;
    }
    return true;
  ...
  const handleCheck = async () => {
    ...
    if (!sanitized.rooms || Object.keys(sanitized.rooms).length === 0) {
      Alert.alert(
        'No measurements',
        'Enter at least one Ceiling/Wall with valid dimensions.',
      );
      return;
    }
    ...
    try {
      ...
      navigation.navigate({
        ...
        params: { rename: { tab: activeTab, from: type, to: roomKeyToSave } },
        merge: true,
      });
    } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView
        ...
        key={nameOfTheRoom || type} // Ensures re-render when room changes
      >
        {/* Interior & Room-Like */}
            ...
            <View style={styles.radioRow}>
              ...
              >
                <View
                  style={[
                    styles.checkbox,
                    roomDefaultMode === MODE.REPAINT && styles.checkboxSelected,
                  ]}
                />
                <Text
                  style={[
            ...
            {sections.map((section, index) =>
              section.type === 'ceiling' ? (
                <View key={`ceiling-${index}`} style={styles.section}>
                  ...
                  {/* Windows, Doors, Cupboards */}
                  {/* ... (same as before) */}
                </View>
              ) : (
                <View key={`wall-${index}`} style={styles.section}>
                  {/* Similar structure for wall */}
                  {/* ... copy from above or keep your original JSX */}
                </View>
              ),
            )}
        ...
        {/* Exterior */}
        {activeTab === 'Exterior' && <>{/* Similar structure */}</>}
