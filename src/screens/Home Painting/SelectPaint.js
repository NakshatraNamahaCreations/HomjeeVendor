...
//     try {
//       setLoading(true);
//       const response = `${API_BASE_URL}${API_ENDPOINTS.GET_QUOTATION
...
//     console.log("canAddAdditional", canAddAdditional);
...
//       try {
//         const { data } = await axios.post(
//           `${API_BASE_URL}${API_ENDPOINTS.UPDATE_QUOTE_PRICING
//           }${encodeURIComponent(quoteId)}/rooms/${encodeURIComponent(
//             roomName,
//           )}/pricing`,
//           payload,
...
const SelectPaint = () => {
  ...
  const fetchQuote = async () => {
    try {
      setLoading(true);
      const response = `${API_BASE_URL}${API_ENDPOINTS.GET_QUOTATION}${encodeURIComponent(
        quoteId,
      )}`;
      const { data } = await axios.get(response);
      const quote = data?.data?.quote || data?.data || null;
      ...
      if (quote) {
        setQuoteLines(Array.isArray(quote.lines) ? quote.lines : []);
        setLoadedAdditionalServices(Array.isArray(quote.lines) ? quote.lines : []);
      }
    } catch (error) {
      console.error('Failed to fetch quote', error);
    } finally {
  ...
  const getServicesForSurface = (roomNameX, surfaceTitle) => {
    const room = (loadedAdditionalServices || []).find(
      r => norm(r?.roomName) === norm(roomNameX),
    );
    const list = (room?.additionalServices || []).filter(
      s => norm(s.surfaceType) === norm(surfaceTitle),
    );
  ...
  useEffect(() => {
    try {
      ...
      const savedC = (existingLine?.breakdown || []).filter(b => b.type === 'Ceiling');
      const savedW = (existingLine?.breakdown || []).filter(b => b.type === 'Wall');
      const savedM = (existingLine?.breakdown || []).filter(b => b.type === 'Measurement');

      setCeilings(prev => hydrateFromLine(prev, 'Ceiling', savedC));
      setWalls(prev => hydrateFromLine(prev, 'Wall', savedW));
      setMeasurements(prev => hydrateFromLine(prev, 'Measurement', savedM));
    } catch (e) {
      console.log('hydrate useEffect error', e);
    }
  }, [existingRoom, existingLine, paintOptions]);
  ...
  /**
   * ✅ Shared saver used by:
   * - Done (navigate back)
   * - Additional Services press (silent save then navigate)
   */
  const saveRoomPricing = async ({ navigateBack }) => {
    try {
      if (resLoading) return false;
      setResLoading(true);
      ...
      const cLines = ceilings.map(it => mk('Ceiling', it)).filter(Boolean);
      const wLines = walls.map(it => mk('Wall', it)).filter(Boolean);
      const mLines = measurements.map(it => mk('Measurement', it)).filter(Boolean);
      ...
      const { data } = await axios.post(
        `${API_BASE_URL}${API_ENDPOINTS.UPDATE_QUOTE_PRICING}${encodeURIComponent(
          quoteId,
        )}/rooms/${encodeURIComponent(roomName)}/pricing`,
        payload,
      );
      ...
      // refresh quote so additional services + selections render updated
      fetchQuote();

      if (navigateBack) navigation.goBack();
      return true;
    } catch (err) {
      console.log('save pricing error', err?.response?.data || err);
      Alert.alert(
        'Error',
        err?.response?.data?.message || 'Failed to save pricing',
      );
  ...
  const onDone = async () => {
    await saveRoomPricing({ navigateBack: true });
  };

  const renderAdditionalServices = (roomNameX, surfaceTitle) => {
    const servicesForThisLine = getServicesForSurface(roomNameX, surfaceTitle);
    if (!servicesForThisLine.length) return null;
    ...
    return (
      <View style={{ marginTop: 6 }}>
        {servicesForThisLine.map((service, index) => {
          ...
          const amount = Number(service.total || 0);
          return (
            <View key={`${service.serviceType}-${index}`} style={{ marginTop: 4 }}>
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>
                  {service.materialName || service.serviceType || 'Additional Service'}
                </Text>
                <Text style={styles.detailValue}>{fmtCalc(qty, rate, amount)}</Text>
              </View>
            </View>
          );
        })}
  ...
  const renderCard = (item, idx, type) => {
    ...
    const modeText = item.mode === 'FRESH' ? 'Fresh Paint' : 'Repaint with primer';
    ...
    const servicesForThisLine = getServicesForSurface(existingRoom?.name || roomName, title);
    const hasWithout = servicesForThisLine.some(s => !s.withPaint);
    ...
    // local paint presence (NEW gating)
    const localSelPaint = getSelectedPaintForType(type);
    const kind =
      type === 'ceiling' ? 'Ceiling' : type === 'wall' ? 'Wall' : 'Measurement';

    // ✅ enable after vendor selects paint (and item not deselected)
    const canAddAdditional = !!localSelPaint && checked && !resLoading;

    const surfaceRef = { type: kind, index: idx + 1, mode: item?.mode ?? null };

    // build breakdown item for passing to AdditionalService
    const mkLine = (k, it) => {
      const sel =
        k === 'Measurement'
          ? selectedPaintOthers
      ...
      const labelType = k === 'Measurement' ? 'measurements' : k.toLowerCase();
      const surf = norm(surfaceLabelFor(labelType, it));

      const originalSqft = Math.max(0, net(it));
      const reduceX = reduceBySurface.get(surf) || 0;
      const zeroAllX = zeroAllBySurface.has(surf);

      let unit = base + (needsPutty ? PUTTY_RATE_PER_SQFT : 0);
      let sqftVal;

      if (zeroAllX) {
        sqftVal = originalSqft;
        unit = 0;
      } else {
        sqftVal = Math.max(0, originalSqft - reduceX);
      }
    ...
    const cLines = ceilings.map(it => mkLine('Ceiling', it)).filter(Boolean);
    const wLines = walls.map(it => mkLine('Wall', it)).filter(Boolean);
    const mLines = measurements.map(it => mkLine('Measurement', it)).filter(Boolean);
    const breakdown = [...cLines, ...wLines, ...mLines];
    ...
    const thisBreakdownItem = typed[idx] || null;

    const handleAdditionalPress = async () => {
      try {
        if (!localSelPaint) {
          Alert.alert('Select paint', 'Please select paint first.');
          return;
        }
        if (!checked) {
          Alert.alert('Select area', 'Please enable this surface first.');
          return;
        }
        const ok = await saveRoomPricing({ navigateBack: false });
        if (!ok) return;
        ...
        console.log('handleAdditionalPress error', e);
      }
    };
    ...
    return (
      <View key={item.id} style={styles.paintCard}>
        ...
        {/* show existing saved additional services list (only when present) */}
        {renderAdditionalServices(existingRoom?.name || roomName, title)}
        ...
        >
          <Text style={styles.additionalText}>
            {resLoading ? 'Saving...' : 'Additional Services'}
          </Text>
          <Entypo name="chevron-with-circle-right" size={18} color="#FF0000" />
        </TouchableOpacity>
      </View>
        ...
        {(isPureOthersRoom || !!measurements.length) && (
          <>
            ...
            <PaintDropdown
              ...
              options={paintOptions}
            />
            {measurements.map((item, idx) => renderCard(item, idx, 'measurements'))}
          </>
        )}
      </ScrollView>
