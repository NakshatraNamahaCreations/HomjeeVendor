...
const SelectPaint = () => {
  ...
  //               {service.materialName || 'No Material'}
  //             </Text>
  ...
  // NEW: render additional services + base/without summary
  ...
  const renderAdditionalServices = (roomName, surfaceTitle, baseCtx) => {
    ...
    // if ANY service is without paint => base is removed for this surface
    const hasWithout = servicesForThisLine.some(s => !s.withPaint);

    // baseCtx contains the original/effective numbers from renderCard
    const {
      baseOriginalSqft,
      baseOriginalAmount,
      baseOriginalRate,
    ...
    return (
      <View style={{ marginTop: 6 }}>
        {/* Summary line about base row */}
        {/* {hasWithout ? (
          <Text style={styles.asSummaryWithout}>
            {`Without Paint — ${surfaceTitle} sqft=0, price=0`}
          </Text>
        ) : (
          <Text style={styles.asSummaryWith}>
            {`Base row remains — ${fmtMoney(baseOriginalAmount || 0)}`}
          </Text>
        )} */}

        {/* List the extra lines */}
        {servicesForThisLine.map((service, index) => {
          ...
          return (
            ...
            >
              ...
              {/* <Text style={styles.asSummaryWith}>({service.serviceType})</Text> */}
              {/* <Text
                style={{
                  fontSize: 12,
                  color: service.withPaint ? '#066a36' : '#b22222',
                  fontFamily: 'Poppins-SemiBold',
                }}
              >
                ({service.withPaint ? 'With Paint' : 'Without Paint'})
          ...
          {/* {rateDetail && (
            <Text
              style={{
                ...
                color: hasWithout ? '#888' : 'green',
                textDecorationLine: hasWithout ? 'line-through' : 'none',
              }}
            >
