...
const RoomMeasurementScreen = () => {
  ...
  //       // Ensure at least one ceiling and one wall
  //       const hasCeiling = cs.length > 0;
  //       const hasWall = ws.length > 0;

  //       const finalCeilings = hasCeiling ? cs : [makeCeiling('Ceiling 1')];
  //       const finalWalls = hasWall ? ws : [makeWall('Wall 1')];

  //       return [...finalCeilings, ...finalWalls];
  //     }
  ...
  //     // For Others (non-fixed), hydrate ceilings and walls
  //     const cs = (room.ceilings || []).map((c, i) => ({
  //       type: 'ceiling',
  //       width: asStr(c.width),
  //       height: asStr(c.height),
  ...
  //     prev.map(s => ({
  //       ...s,
  //       mode: s.customMode ? s.mode : MODE.REPAINT,
  //     })),
  //   );
  // };
