...
const StartMeasurement = () => {
  ...
  useEffect(() => {
    try {
      if (defaultTabParam) setActiveTab(defaultTabParam);
    } catch (e) {
      console.log('set tab error', e);
    }
  }, [defaultTabParam]);
  ...
  const displayList = useMemo(() => {
    ...
    // Predefined rooms
    const predefined = (tabsData?.[tab] || []).map(name => ({
      name,
      origin: 'PREDEFINED',
    }));

    // Custom rooms
    const custom = (customRoomsByTab?.[tab] || []).map(name => ({
      name,
      origin: 'CUSTOM',
    }));
    ...
    // Auto-discovered rooms (saved in DB but not in predefined or custom)
    const auto = Object.entries(roomsData)
      .filter(([name, room]) => {
        const belongsTo =
          normalize(room?.sectionType) || NAME_SECTION.get(name);
        if (belongsTo !== tab) return false;

        // Skip if already in predefined or custom
        if (
          predefined.some(p => p.name === name) ||
          custom.some(c => c.name === name)
        ) {
          return false;
        }
  ...
  const navigateToScreen = (type, origin = 'UNKNOWN') => {
    ...
    setTimeout(() => {
      ...
      setLoading(false); // Stop the loader after navigation
    }, 500);
  };
        ...
        <ScrollView contentContainerStyle={{ padding: 16, marginBottom: 100 }}>
          {displayList.length === 0 ? (
            ...
            displayList.map(item => {
              const { name: roomName, origin } = item; // ← destructured
              ...
              return (
                <View key={roomName} style={styles.rowCard}>
                  <TouchableOpacity
                    ...
                    onPress={() => navigateToScreen(roomName, origin)}
                  >
