...
const RoomMeasurementScreen = () => {
  ...
  const onHeaderBack = React.useCallback(() => {
    try {
      ...
      console.log('onHeaderBack err', e);
    }
  }, [isDirty, navigation]);
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      return () => sub.remove();
    }, [showConfirmPopup, isDirty, navigation]),
  );
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <View style={[styles.flexRow, { justifyContent: 'space-between' }]}>
              {/* Discard */}
              <TouchableOpacity
                style={[styles.cancelButton, { opacity: saving ? 0.6 : 1 }]}
                disabled={saving}
                onPress={() => {
                  try {
                    setShowConfirmPopup(false);
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  ...
                  try {
                    setSaving(true);
                    // If you also want the page overlay loader, uncomment:
                    // if (typeof setLoading === 'function') setLoading(true);

                    await handleCheck(); // your existing save/validate
