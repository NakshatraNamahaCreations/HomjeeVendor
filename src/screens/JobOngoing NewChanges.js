...
const JobOngoing = () => {
  ...
  const getStatusText = () => {
    if (!latestChange) return 'Status: —';

    if (isApproved) {
      const who = latestChange.approvedBy
        ? `${latestChange.approvedBy.charAt(0).toUpperCase() +
        latestChange.approvedBy.slice(1)
        }`
        : 'Unknown';
    ...
    if (isRejected) {
      const who = latestChange.rejectedBy
        ? `${latestChange.rejectedBy.charAt(0).toUpperCase() +
        latestChange.rejectedBy.slice(1)
        }`
        : 'Unknown';
  ...
  // Base amounts
  // const originalTotal =
  //   bd.originalTotalAmount && bd.originalTotalAmount > 0
  //     ? bd.originalTotalAmount
  //     : bd.bookingAmount ||
  //       (bd.priceChanges?.length ? bd.priceChanges[0].proposedTotal : 0);

  const originalTotal = bd.bookingAmount || bd.originalTotalAmount || 0;

  // ✅ Calculate cumulative approved change
  const totalApprovedDelta = approvedChanges.reduce(
    (sum, change) =>
      change.scopeType === 'Reduced'
        ? sum - (change.adjustmentAmount || 0)
        : sum + (change.adjustmentAmount || 0),
    0,
  );
  // const totalApprovedDelta = approvedChanges.reduce(
  //   (sum, change) => sum + (change.adjustmentAmount || 0),
  //   0,
  // );

  // ✅ Latest approved total (if exists)
  const latestApprovedChange =
    approvedChanges.length > 0
      ? approvedChanges[approvedChanges.length - 1]
      : null;

  const approvedTotal = latestApprovedChange
    ? latestApprovedChange.proposedTotal
    : originalTotal;

  // 🧮 Pending Delta (if currently pending)
  const pendingDelta =
    isPending && latestChange ? latestChange.proposedTotal - approvedTotal : 0;

  const currentFinalTotal = bd.finalTotal || bd.bookingAmount || originalTotal;
  const paid = bd.paidAmount || 0;

  // // For display: what's the "current approved total"?
  // const approvedTotal = hasApprovedChange
  //   ? approvedChanges[approvedChanges.length - 1].proposedTotal
  //   : currentFinalTotal;

  // // Deltas
  // const pendingDelta = isPending
  //   ? latestChange.proposedTotal - currentFinalTotal
  //   : 0;

  const approvedDelta = hasApprovedChange
    ? approvedTotal -
    (approvedChanges.length > 1
      ? approvedChanges[approvedChanges.length - 2].proposedTotal
      : originalTotal)
    : 0;

  const disableAllTheHell = bd.paymentStatus === 'paid';
  ...
  const showFianlPayment =
    vendorDataContext?.vendor?.serviceType === 'house-painter' &&
    bd.secondPayment.status === 'paid' &&
    bd.finalPayment.status === 'pending';

  const isOngoing = 'project ongoing';

  const isEdited = !!bd.hasPriceUpdated;
  const priceGateDisabled = isEdited && latestChange?.status === 'pending';
  const showEndJobEnabled =
    isOngoing && bd.paymentStatus === 'Partially Completed';
  ...
  // const getStatusText = () => {
  //   try {
  //     const bd = leadDataContext?.bookingDetails.priceChanges[0];
  //     if (!bd) return 'Status: —';
  ...
  //     (bd.currentTotalAmount > 0 ? bd.currentTotalAmount : bd.bookingAmount) ??
  //     0,
  // );
  ...
  // // ............FROM HERE.............
  // const showPriceBanner = bd?.hasPriceUpdated;
  ...
  // const oldTotalForApproved = Number.isFinite(lastApproved?.baseAtProposal)
  //   ? Number(lastApproved.baseAtProposal)
  //   : Number.isFinite(bd.currentTotalAmount) && bd.currentTotalAmount > 0
  //   ? bd.currentTotalAmount
  //   : originalQuote;
  ...
  // const approvedChangeDelta = Number.isFinite(lastApproved?.delta)
  //   ? Number(lastApproved.delta)
  //   : Number.isFinite(bd.finalTotal) && bd.finalTotal > 0
  //   ? bd.finalTotal - originalQuote
  //   : effectiveTotal - originalQuote;
  ...
  //   isPending: isPending,
  //   hasApprovedDelta: hasApprovedDelta,
  //   oldTotalForApproved: oldTotalForApproved,
  //   approvedChangeDelta: approvedChangeDelta,
  //   approvedNewTotal: approvedNewTotal,
  //   pendingDelta: pendingDelta,
  //   pendingNewTotal: pendingNewTotal,
  //   currentTotalAmount: bd.currentTotalAmount,
  //   finalTotal: bd.finalTotal,
  // });
          ...
          {latestChange && (
            <View style={styles.headerBlock}>
              <Text style={{ fontFamily: 'Poppins-SemiBold', fontSize: 12 }}>
                Amount {latestChange.scopeType}: ₹
                {Math.abs(pendingDelta || approvedDelta)}
              </Text>
              <Text
                style={{
                  fontFamily: 'Poppins-SemiBold',
            ...
            leadDataContext.bookingDetails?.status === 'Hired' ||
              ...
              >
                ...
                >
                  ...
                  {/** 🟠 CASE 1 & 3: Pending (Customer/Admin approval) */}
                  {isPending && (
                    <View>
                      {hasApprovedChange ? (
                        <>
                          ...
                          <View style={styles.rowBetween}>
                            ...
                            <Text style={styles.amountDue}>
                              {currency(approvedTotal - approvedDelta)}
                            </Text>
                          </View>
                          ...
                          <View style={styles.rowBetween}>
                            ...
                            <Text
                              style={[
                                styles.amountDue,
                                { color: approvedDelta < 0 ? 'red' : 'green' },
                              ]}
                            >
                              {approvedDelta < 0 ? '-' : '+'}{' '}
                              {currency(Math.abs(approvedDelta))}
                            </Text>
                          </View>
                          ...
                          <View style={styles.rowBetween}>
                            ...
                            <Text style={styles.amountDue}>
                              {currency(latestChange.proposedTotal)}
                            </Text>
                          </View>
                        </>
                      ) : (
                  ...
                  {/** 🔴 CASE 4 & 8: Rejected after approval (customer/admin) */}
                  {isRejected && hasApprovedChange && (
                    <View>
                      <View style={styles.rowBetween}>
                        <Text style={styles.amountLabel}>Old Total </Text>
                        <Text style={styles.amountDue}>
                  ...
                  {/** 🔴 CASE 5: First-time rejection (no previous approvals) */}
                  {isRejected && !hasApprovedChange && (
                    <View>
                      <View style={styles.rowBetween}>
                        <Text style={styles.amountLabel}>Total Amount</Text>
                      ...
                      <View style={styles.rowBetween}>
                        ...
                        <Text style={styles.amountDue}>
                          {currency(currentFinalTotal - paid)}
                        </Text>
                      </View>
                    </View>
                  )}
                  {/** 🟢 CASE 2 & 6: Approved (customer/admin) */}
                  {isApproved && (
                    <View>
                      <View style={styles.rowBetween}>
...
//                             {
//                               color:
//                                 totalApprovedDelta < 0 ? 'red' : 'green',
//                             },
//                           ]}
//                         >
//                           {totalApprovedDelta < 0 ? '-' : '+'}{' '}
