...
const JobOngoing = () => {
  ...
  const getStatusText = () => {
    try {
      ...
      // Prefer explicit tri-state; fallback:
      // - true  -> approved
      // - false + hasPriceUpdated -> pending
      // - false + !hasPriceUpdated -> approved (nothing to approve)
      const state =
      ...
      if (state === 'approved') {
        ...
        const when =
          bd.priceApprovedTime ||
          (bd.priceApprovedDate
            ? new Date(bd.priceApprovedDate).toLocaleString()
            : '');
      ...
      if (state === 'rejected') {
        ...
        const when =
          bd.priceRejectedTime ||
          (bd.priceRejectedDate
            ? new Date(bd.priceRejectedDate).toLocaleString()
            : '');
  ...
  // ---- derive once from context
  const bd = leadDataContext?.bookingDetails ?? {};

  const status = (bd.status || '').trim().toLowerCase(); // e.g. "project ongoing"
  const paymentStatus = (bd.paymentStatus || '').trim().toLowerCase(); // "partial payment", "partially completed", "paid"
  ...
  // request/enable/disable flags based on payment flow (unchanged, but case-safe)
  ...
  // ---- approval state + price gate (memoized)
  const {
    approvalState, // "approved" | "pending" | "rejected"
    priceGateDisabled, // lock actions only while pending after an edit
    endJobEnabledFinal,
    endJobDisabledFinal,
    canRequestNextEnabled,
  } = useMemo(() => {
    try {
      // prefer tri-state; fallback: true→approved, false→pending (only if edited)
      const rawState =
        bd.priceApprovalState ??
        (bd.priceApprovalStatus
          ...
          ? 'pending'
          : 'approved');
      const approvalState = String(rawState).trim().toLowerCase();
      ...
      return {
        ...
        priceGateDisabled: true,
        endJobEnabledFinal: false,
        endJobDisabledFinal: false,
        canRequestNextEnabled: false,
      };
    }
    // deps
  }, [
  ...
  // ---- price edit button availability
  const priceEditLocked = !!bd.hasPriceUpdated && approvalState === 'pending';
        ...
        >
          ...
          {leadDataContext.bookingDetails?.hasPriceUpdated ? (
            <View style={styles.headerBlock}>
              <Text style={{ fontFamily: 'Poppins-SemiBold', fontSize: 12 }}>
                Amount {leadDataContext.bookingDetails?.scopeType} : Rs.{' '}
                {leadDataContext.bookingDetails?.editedPrice}
              </Text>
              <Text
                style={{
                  fontFamily: 'Poppins-SemiBold',
          ...
          leadDataContext.bookingDetails?.status === 'Project Completed' ? (
            ...
            >
              ...
              {leadDataContext.bookingDetails?.priceApprovalStatus ? (
                <View>
                  <View style={styles.rowBetween}>
                    <Text style={styles.label}>Old Total </Text>
                    <Text style={styles.amountBold}>
                      ₹ {leadDataContext?.bookingDetails?.bookingAmount}
                    </Text>
                  </View>
                  <View style={styles.rowBetween}>
                    ...
                    <Text
                      style={[
                        ...
                        {
                          color:
                            leadDataContext.bookingDetails?.scopeType ===
                            'Reduced'
                              ? 'red'
                              : 'green',
                        },
                    ...
                    >
                      {leadDataContext.bookingDetails?.scopeType === 'Reduced'
                        ? '-'
                        : '+'}{' '}
                      ₹ {leadDataContext.bookingDetails?.editedPrice}
                    </Text>
                  ...
                  <View style={styles.rowBetween}>
                    ...
                    <Text style={styles.amountDue}>
                      ₹ {leadDataContext.bookingDetails?.newTotal}
                      <TouchableOpacity
                ...
                <View>
                  <View style={styles.rowBetween}>
                    <Text style={styles.label}>Total Amount</Text>
                    <Text style={styles.amountBold}>
                      ₹ {leadDataContext?.bookingDetails?.bookingAmount ?? 0}{' '}
                      {isAllowedStatus &&
                        (canEditPrice ? (
                          <TouchableOpacity
                            onPress={() => setSecondModalVisible(true)}
                          ...
                          <MaterialIcons
                            ...
                            accessibilityLabel={
                              priceEditLocked
                                ? 'Price edit locked until approval'
