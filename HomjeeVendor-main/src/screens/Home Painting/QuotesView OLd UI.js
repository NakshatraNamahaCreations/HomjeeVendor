...
export default function QuotesView({ quote = SAMPLE_QUOTE }) {
  ...
  const roomsObj = measurementData?.rooms || {};
  ...
  const hasEstimate =
    !!measurementData && Object.keys(measurementData?.rooms || {}).length > 0;
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.summaryRow}>
          <Card style={{ flex: 1, marginRight: 8 }}>
            ...
            <Text style={styles.total}> {rupee(quotes?.totals?.grandTotal ?? 0)}</Text>
            ...
            <View style={styles.badgeRow}>
              <View style={styles.badge}>
                <Text style={styles.badgeText}>
                  Project Duration:  {quotes?.days ?? 0} {(quotes?.days ?? 0) === 1 ? "Day" : "Days"}
                </Text>
              </View>
            </View>
          </Card>
